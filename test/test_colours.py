...
class ColourTests(UnitTestWrapper):
    ...
    def test_getColourCode4Bit(self):
        ...
        self.assertEquals(c.RED[c.VAL_KEY], c._getColourCode(c.RED) )
        self.assertEquals(c.BLACK[c.VAL_KEY], c._getColourCode("black"))
        self.assertEquals(c.MAGENTA[c.VAL_KEY], c._getColourCode("m"))
        for colour in c.COLOURS:
            self.assertEquals(colour[c.VAL_KEY], c._getColourCode(colour) )
            self.assertEquals(colour[c.VAL_KEY], c._getColourCode(colour[c.NAME_KEY]) )
            self.assertEquals(colour[c.VAL_KEY], c._getColourCode(colour[c.CODE_KEY]) )
    ...
    def test_getColourCodeBg4Bit(self):
        c = prompty.colours.Colours(None)
        for colour in c.COLOURS:
            self.assertEquals(int(colour[c.VAL_KEY])+c.BG_OFFSET, 
    ...
    def test_getColourCode8Bit(self):
        c = prompty.colours.Colours(None)
        self.assertEquals("38;5;145", c._getColourCode("145") )
        self.assertEquals("38;5;0", c._getColourCode("0") )
        self.assertEquals("38;5;255", c._getColourCode("255") )
        self.assertRaises(ValueError, c._getColourCode, "256")
        self.assertRaises(ValueError, c._getColourCode, "0x456")
        self.assertEquals("38;5;16", c._getColourCode("#000") )
        self.assertEquals("38;5;196", c._getColourCode("#f00") )
        self.assertEquals("38;5;46", c._getColourCode("#0f0") )
        self.assertEquals("38;5;21", c._getColourCode("#00f") )
        self.assertRaises(ValueError, c._getColourCode, "#bat")
        self.assertEquals("38;5;231", c._getColourCode("#gff") )
        self.assertEquals("38;5;16", c._getColourCode("#g00") )
        self.assertEquals("38;5;232", c._getColourCode("#g05") )
        self.assertEquals("38;5;239", c._getColourCode("#g4e") )
        self.assertEquals("38;5;255", c._getColourCode("#gee") )

    def test_getColourCodeBg8Bit(self):
        c = prompty.colours.Colours(None)
        self.assertEquals("48;5;145", c._getColourCode("145",c.BACKGROUND) )

    def test_getColourCode24Bit(self):
        c = prompty.colours.Colours(None)
        self.assertEquals("38;2;0;0;0", c._getColourCode("#000000") )
        self.assertEquals("38;2;1;2;3", c._getColourCode("#010203") )
        self.assertEquals("38;2;255;255;255", c._getColourCode("#ffffff") )
        self.assertEquals("38;2;0;0;0", c._getColourCode("0,0,0") )
        self.assertEquals("38;2;1;2;3", c._getColourCode("1,2,3") )
        self.assertEquals("38;2;255;255;255", c._getColourCode("255,255,255") )
        self.assertRaises(ValueError, c._getColourCode, "0,0")

    def test_getColourCodeBg24Bit(self):
        c = prompty.colours.Colours(None)
        self.assertEquals("48;2;1;2;3", c._getColourCode("#010203",c.BACKGROUND) )
        self.assertEquals("48;2;1;2;3", c._getColourCode("1,2,3",c.BACKGROUND) )
    ...
    def test_getStyleCode(self):
        ...
        for style in c.STYLES:
            self.assertEquals(c._getStyleCode(style), style[c.VAL_KEY])
            self.assertEquals(c._getStyleCode(style[c.NAME_KEY]), style[c.VAL_KEY])
            self.assertEquals(c._getStyleCode(style[c.CODE_KEY]), style[c.VAL_KEY])
